...
setInterval(() => {
  ...
  ajax(
    'GET',
    `${apiUrl}/newer/${lastPost.id}`,
    {},
    {
      onStart: () => {},
      onSuccess: (postData) => {
        newPosts = JSON.parse(postData);
        if (newPosts.length) {
          newPosts.forEach((element) => {
            if (postsEl.childElementCount) {
              postsEl.insertBefore(makePostEl(element), postsEl.children[0]);
            } else {
              postsEl.appendChild(element);
            }
            posts.unshift(element);
          });
        }
      },
    },
